...
| **Call-graph library** | **SootUp** (primary), WALA (fallback/comparison) | SootUp is the modern, actively maintained successor to Soot; proven precedent (Steady, Endor, dep-scan use Soot/WALA-class tools for Java). **Confirmed 2026-08-01** (see `docs/reachability-caveats.md`): the `invokedynamic`/lambda call-site gap is real, not just a secondary-source claim — reproduced against a dedicated fixture and by inspecting `sootup.callgraph-1.1.2.jar`'s contents directly (no lambda/invokedynamic-handling classes exist in it at all). **Still unverified** (flagged 2026-08-01 from `docs/competitive-landscape-commercial-technical.md` Part 2, not yet independently re-checked): the version pin at `reachability/build.gradle.kts` (1.1.2) was justified only against 1.3.0/2.0.0 and appears stale against the actual current release (reported as 3.0.0), and whether a newer SootUp version actually adds invokedynamic resolution; RTA (already in the same SootUp API family) is reported as an available, low-effort precision upgrade over CHA-only that hasn't been adopted. The documented WALA fallback has no actual dependency in `reachability/build.gradle.kts` today. |
