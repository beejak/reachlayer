...
| **Call-graph library** | **SootUp** (primary), WALA (fallback/comparison) | SootUp is the modern, actively maintained successor to Soot; proven precedent (Steady, Endor, dep-scan use Soot/WALA-class tools for Java). **Fixed 2026-08-01** (see `docs/reachability-caveats.md`): the `invokedynamic`/lambda call-site gap — SootUp's stock CHA algorithm dropped every such call edge — was confirmed by reproducing it against a dedicated fixture and inspecting `sootup.callgraph-1.1.2.jar`'s contents directly, then fixed via `reachability`'s own `LambdaAwareChaAlgorithm` subclass (an app-side fix, no SootUp upgrade needed). **Still unverified** (flagged 2026-08-01 from `docs/competitive-landscape-commercial-technical.md` Part 2, not yet independently re-checked): the version pin at `reachability/build.gradle.kts` (1.1.2) was justified only against 1.3.0/2.0.0 and appears stale against the actual current release (reported as 3.0.0). **The "RTA is a low-effort precision upgrade" half of that same flag, checked 2026-08-01: not a free win, and not even a strict superset of today's fix.** Decompiling `RapidTypeAnalysisAlgorithm.resolveCall` shows it has no `invokedynamic` special-case at all (unlike CHA's explicit early-return) — it falls through to `getMethodSignature()`, which for a lambda dispatch site returns the SAM interface's abstract method (e.g. `Supplier.get()`), then relies on RTA's own instantiated-class tracking to resolve the real implementor; since a lambda's synthetic class is never explicitly `new`'d in the bytecode SootUp sees, RTA would likely fail to resolve real lambda dispatch too — just silently, via a different mechanism, not via a documented, deliberate early-return like CHA's. `RapidTypeAnalysisAlgorithm` also doesn't extend `ClassHierarchyAnalysisAlgorithm` (it's a sibling under `AbstractCallGraphAlgorithm`), so today's `LambdaAwareChaAlgorithm` fix would need a separate `LambdaAwareRtaAlgorithm` sibling, not just a swap. More fundamentally, RTA's whole premise (narrow reachability to instantiated types) trades away some of CHA's deliberate over-approximation margin — the same margin PLAN.md §9 risk 1 relies on to "prefer under-claiming unreachability." Adopting RTA is a real precision/safety tradeoff to weigh deliberately, not a drop-in upgrade. The documented WALA fallback has no actual dependency in `reachability/build.gradle.kts` today. |
...
| 11 | **Black Duck Detect may already emit its own reachability verdict** (`--detect.impact.analysis.enabled`, per `docs/competitive-landscape-oss.md`). The `blackduck` connector now parses an optional `vendorReachability` field and `ReachabilityTagger`/`MarkdownReportFormatter` surface a disagreement between it and Reachlayer's own CHA tag rather than silently preferring one — but the field name/shape is Reachlayer's own invented representation, not verified against a real export (§9 risk 6 forbids using one to check). | Low, now that both signals are surfaced side-by-side rather than one being silently discarded — the residual risk is just that the field name might not match a real export's, meaning the mechanism sits unused until a real customer confirms/corrects the shape. | Mechanism implemented (`docs/connectors.md`) per the "layer, never replace" stance in §2. Exact wire-format verification remains open, blocked on real-vendor-data access this project deliberately does not pursue. |
